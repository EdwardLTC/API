        ...
        protected override void OnModelCreating(ModelBuilder modelBuilder)
        {
            ...
            modelBuilder.Entity<ClothesProperty>(entity =>
            {
                ...
                entity.HasOne(d => d.IdNavigation)
                    .WithOne(p => p.ClothesProperty)
                    .HasForeignKey<ClothesProperty>(d => d.Id)
                    .OnDelete(DeleteBehavior.ClientSetNull)
            ...
            modelBuilder.Entity<Person>(entity =>
            {
                ...
                entity.Property(e => e.Psw).HasMaxLength(50);

                entity.HasMany(d => d.Idclothes)
                    .WithMany(p => p.Idpeople)
                    .UsingEntity<Dictionary<string, object>>(
                        "Favorite",
                        l => l.HasOne<Clothe>().WithMany().HasForeignKey("Idclothes").OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_Favorites_Clothes"),
                        r => r.HasOne<Person>().WithMany().HasForeignKey("Idpeople").OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_Favorites_People"),
                        j =>
                        {
                            j.HasKey("Idpeople", "Idclothes");

                            j.ToTable("Favorites");

                            j.IndexerProperty<int>("Idpeople").HasColumnName("IDpeople");

                            j.IndexerProperty<int>("Idclothes").HasColumnName("IDclothes");
                        });
            });
